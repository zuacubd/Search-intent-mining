...
public class JelinkMercerDocumentLikelihoodModelLanguageModel {

	private Map termUnigramPostingList;
	private Integer numTermUnigramFreq;
	private Map subtopicLMDLM;
	
	public Map getSubtopicDLM() {
		return this.subtopicLMDLM;
	}

	public JelinkMercerDocumentLikelihoodModelLanguageModel(Map termUnigramPostingList, Integer numTermUnigramFreq, String topicString, Map subtopics) {
		
		this.termUnigramPostingList = termUnigramPostingList;
		this.numTermUnigramFreq = numTermUnigramFreq;
		
		this.subtopicLMDLM = this.getEstimatedLMDLMFeatureWeight(topicString, subtopics);
	}

	private Map getEstimatedLMDLMFeatureWeight(String topicString, Map subtopics) {

		Map documentLikelihoodModelFeatureWeight = new HashMap();

		Double dlmFeatureWeight;
		...
		for(Object subtopic:subtopics.keySet()) {
			...
			dlmFeatureWeight = this.getDocumentLikeLihoodModelFeatureWeight(topicString, subtopicString);

			documentLikelihoodModelFeatureWeight.put(subtopic, dlmFeatureWeight);
		}
		
		return documentLikelihoodModelFeatureWeight;
	}

	private Double getDocumentLikeLihoodModelFeatureWeight(String topicString,
			String subtopicString) {
		...
		try {
			...
			for(Object subtopicTerm:subtopicTermMap.keySet())
			{
				...
				termProbability = alpha.doubleValue() * probabilityOfTermGivenTopic.doubleValue() + (1.0 - alpha.doubleValue()) * probabilityOfTermGivenCorpus.doubleValue();
